/*
 * img2d88.c version 2.2 part of the vfman 2.2 package
 *
...
/*
 * img2d88 - converts an old type vfloppy image to the d88 format
 * 
 * img2d88 <vfImageName> <d88ImageName>
 */
...
{	
	...
	{
		fprintf(stderr, "%s <imgImageName> <d88ImageName>\n", argv[0]);
		fprintf(stderr, "img2d88 2.3 (c)2023 F.J. Kraan\n");
		exit(1);
	}

	if (checkFileExistance(pArgData->imageName) == 0)
	{
		printf("img image '%s' not found, exiting.\n", argv[1]);
	}
	char *imgImageName = pArgData->imageName;
	FILE *imgImageFile = NULL;
	...
	imgImageFile = openFileRO(imgImageName);	// open image
	pFileData->file = openImageRW((char *)pFileData->fileName);	// open file

	int i, j, k = 0, imgOffset = 0x8000;
	...
	for (i = 0; i < BLOCKS_ON_DISK; i++)
	{
		...
		{
			readDisk((char *)sector, k * SECTOR + imgOffset, SECTOR, imgImageFile);
			writeDisk((char *)sector, getBlockSectorLocation(i, j), SECTOR, pFileData->file);
			k++;
		}
	}
	
	fclose(imgImageFile);		// close image
